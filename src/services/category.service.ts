...
export class CategoryService extends CategoryHelpers{
    ...
    public async getLanCatExam(req:Request, res:Response){
        const Category: ICategory = await super.GetCategory({_id: req.params.id});

        Language.find({category: Category},(err:Error, cat: ICategory[])=>{
            if(err){
